...
def test_symmetric() -> None:
    ...
    """
    pairs = list(zip(range(100), range(100, 201)))
    assert [mul(x, y) for x, y in pairs] == [mul(y, x) for x, y in pairs]
...
def test_distribute() -> None:
    ...
    """
    triplets = list(zip(range(100), range(100, 201), range(200, 301)))
    assert [mul(z, add(x, y)) for x, y, z in triplets] == [
        add(mul(z, x), mul(z, y)) for x, y, z in triplets
    ]
...
def test_other() -> None:
    ...
    """
    triplets = list(zip(range(100), range(100, 201), range(200, 301)))
    assert [max(x, max(y, z)) for x, y, z in triplets] == [
        max(max(x, y), z) for x, y, z in triplets
    ]
